//! Criterion benchmarks for cockpitctl-conform validation.
//!
//! Run with: `cargo bench -p cockpitctl-conform`
//!
//! These benchmarks measure conformance checking performance with various report sizes.
...
use cockpitctl_types::{
    ...
    PolicySnapshot, Presence, RunInfo, SensorReport, SensorSummary, Severity, ToolInfo, Verdict,
...
/// Benchmark `conform_single` with all checks enabled, varying finding counts.
fn bench_conform_single_all_checks(c: &mut Criterion) {
    let mut group = c.benchmark_group("conform_single_all_checks");
...
/// Benchmark `conform_single` with only schema validation (minimal checks).
fn bench_conform_single_schema_only(c: &mut Criterion) {
    let mut group = c.benchmark_group("conform_single_schema_only");
...
/// Benchmark `validate_cockpit_schema` with varying report sizes.
fn bench_validate_cockpit_schema(c: &mut Criterion) {
    let mut group = c.benchmark_group("validate_cockpit_schema");
    ...
    for (num_sensors, num_highlights, label) in test_cases {
        ...
        group.bench_function(label, |b| {
            b.iter(|| validate_cockpit_schema(black_box(&json)));
        });
    }
...
criterion_group!(
    benches,
    bench_conform_single_all_checks,
    bench_conform_single_schema_only,
    bench_validate_cockpit_schema,
);
