...
use cockpitctl_validate::JsonSchemaValidator;
...
fn cmd_ingest(opts: IngestOptions) -> Result<i32> {
    ...
    if result.buildfix.is_some() || buildfix_apply_requested {
        ...
        if result.buildfix.is_none() {
            ...
            match cockpitctl_exec::run_buildfix_actuator(actuator, &request) {
                Ok(out) => {
    ...
    if effective_policy_signing.enabled {
        let key = cockpitctl_exec::load_policy_signing_key(&effective_policy_signing)
    ...
    if !pre_cfg.hooks.is_empty() {
        ...
        let sections = cockpitctl_exec::run_hooks(&pre_cfg.hooks, &report_json, &hook_output)
            .context("run hooks")?;
