...
pub fn run_buildfix_actuator(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        let payload = serde_json::to_vec(request).context("serialize buildfix apply request")?;
        stdin
            .write_all(&payload)
            .context("write stdin for buildfix actuator")?;
    }
    ...
    if !status.success() {
        anyhow::bail!(
            "buildfix actuator exited with status {}: {}",
            status,
    ...
    let mut result: BuildfixActuatorResult =
        serde_json::from_slice(&stdout_bytes).context("parse buildfix actuator output")?;
...
pub fn load_policy_signing_key(cfg: &PolicySigningConfig) -> Result<Option<Vec<u8>>> {
    if let Some(path) = &cfg.key_path {
        let bytes = std::fs::read(path)
            .with_context(|| format!("read policy signing key from {}", path))?;
