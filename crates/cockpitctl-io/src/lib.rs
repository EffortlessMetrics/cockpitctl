...
// ============================================================================
// Operational helpers (hook/buildfix/signing) live in cockpitctl-ops
// ============================================================================

pub use cockpitctl_ops::{
...
mod tests {
    use super::*;
    use cockpitctl_types::{BuildfixActuatorConfig, BuildfixApplyRequest, PolicySigningConfig};
...
"#,
    ...
    fn policy_source_read_error_is_reported() {
        ...
        match policy.load_config() {
            Err(err) => assert!(format!("{:#}", err).contains("read config")),
            Ok(Some(_)) => {
                // Some environments (e.g. root) can still read chmod(000) files.
            }
            Ok(None) => panic!("expected read error or readable config"),
        }
    }
    ...
    fn read_report_bytes_read_error_when_locked() {
        ...
        match source.read_report_bytes("sensor") {
            Err(err) => assert!(format!("{:#}", err).contains("read receipt")),
            Ok(ReportRead::Bytes(_)) => {
                // Some environments (e.g. root) can still read chmod(000) files.
            }
            Ok(_) => panic!("expected read error or readable bytes"),
        }
    }
