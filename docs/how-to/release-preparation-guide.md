...
The cockpitctl release process is automated via GitHub Actions and triggered by pushing a version tag (e.g., `v0.2.0`). The workflow consists of five jobs:
...
# The version should match the tag you plan to create (e.g., 0.2.0 for v0.2.0)
```
...
**Note**: The tag format must be `v*` (e.g., `v0.2.0`, `v1.0.0`). The workflow will verify that the tag version matches the version in [`Cargo.toml`](../../Cargo.toml:17).
...
```
https://github.com/EffortlessMetrics/cockpitctl/releases/tag/v0.2.0
```
